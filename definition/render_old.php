...
@media (max-width: 320px) {
  ...
  .wd-pagination .prev-next {
    ...
    jQuery(function($){
      ...
      function doFilter(search, filter, enter, page){
        if (isLoading) return;
...
function wd_filter_definitions_callback() {
    // Performance optimization: check if this is a valid AJAX request
    if (!defined('DOING_AJAX') || !DOING_AJAX) {
        wp_die();
    }
    ...
    // build query args with performance optimizations
    $args = [
      'post_type'      => 'definition',
      'posts_per_page' => $enter ? 1 : 10,
      'orderby'        => 'title',
      'order'          => 'ASC',
      'paged'          => $paged,
