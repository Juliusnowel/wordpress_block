  ...
  .wd-pagination .prev-next {
    ...
    jQuery(function($){
      var filterTimer, suggestTimer,
        ...
        // Detect language on the page
        var htmlLang = (document.documentElement.lang || '').toLowerCase();
        var hasBodyKo = document.body && document.body.classList.contains('lang-ko');
        var isKO = htmlLang.startsWith('ko') || hasBodyKo || /^\/ko(\/|$)/.test(location.pathname);
        var langSlug = isKO ? 'ko' : 'en';
        var labels = isKO ? {
            all: '전체',
            digits: '0-9',
            definition: '정의',
        ...
        // Search placeholder
        $('#wd-search-input').attr('placeholder', isKO ? '용어 검색…' : 'Search definitions…');

        // Top nav: All / 0-9
        $('.wd-definitions-nav .filter-link[data-filter="all"]').text(labels.all);
        $('.wd-definitions-nav .filter-link[data-filter="0-9"]').text(labels.digits);

        // Category buttons (also normalize slugs so clicks still work)
        $('.wd-category-nav .cat-filter-link[data-cat="all"]').text(labels.all);
        $('.wd-category-nav .cat-filter-link[data-cat="definition"]').text(labels.definition);

        // “reviews” may be authored as `review` in your HTML; normalize both
        $('.wd-category-nav .cat-filter-link[data-cat="review"], .wd-category-nav .cat-filter-link[data-cat="review"]')
        .text(labels.review)
        .attr('data-cat','review');

        $('.wd-category-nav .cat-filter-link[data-cat="news"]').text(labels.news);

        // same for how-tos (handle singular in HTML)
        $('.wd-category-nav .cat-filter-link[data-cat="how-to"], .wd-category-nav .cat-filter-link[data-cat="how-to"]')
        .text(labels.howto)
        .attr('data-cat','how-to');

        // OPTIONAL: On Korean pages the A–Z row isn’t very useful.
        // Hide the letter links and keep just “전체 / 0-9”.
        // if (isKO) {
        // $('.wd-definitions-nav .filter-link').not('[data-filter="all"], [data-filter="0-9"]').hide();
        // }
...
function wd_get_lang_from_request() {
    $lang = isset($_POST['lang']) ? sanitize_key($_POST['lang']) : '';
    if ($lang) return $lang;

    // Fallbacks if nothing was sent
...
function wd_suggest_definitions_callback() {
    ...
    $lang = wd_get_lang_from_request();

    $q = new WP_Query([
        'post_type'        => 'definition',   // suggestions from "definition" CPT
        'post_status'      => 'publish',
        's'                => $s,             // use the search term
        'orderby'          => 'title',
        'order'            => 'ASC',
        'posts_per_page'   => 5,
        'lang'             => $lang,          // Polylang language
        'suppress_filters' => false,          // let PLL hook in
    ]);
...
function wd_filter_definitions_callback() {
    ...
    $lang     = wd_get_lang_from_request();  // <- get the lang
    $read_more = ($lang === 'ko') ? '자세히 보기' : 'Read more';
    
    $prev_label = ($lang === 'ko') ? '‹ 이전' : '‹ Prev';
    $next_label = ($lang === 'ko') ? '다음 ›' : 'Next ›';
    ...
    $args = [
        ...
        'lang'                    => $lang,          // <- key line
        'suppress_filters'        => false,          // <- let Polylang run
    ];
    ...
    if ( $posts ) {
      foreach ( $posts as $p ) {
        ...
        printf(
            '<li>%s<div class="wd-content"><a class="title" href="%s" target="_blank" rel="noopener noreferrer">%s</a><p class="wd-excerpt">%s</p><a class="wd-read-more" href="%s" target="_blank" rel="noopener noreferrer">%s</a></div></li>',
            $thumb,
            esc_url( get_permalink( $p->ID ) ),
            esc_html( $p->post_title ),
            esc_html( $ex ),
            esc_url( get_permalink( $p->ID ) ),
            esc_html( $read_more )   // ← '자세히 보기' 또는 'Read more'
        );
      }
    } else {
    ...
    if ( ! $enter && $q->max_num_pages > 1 ) {
      ...
      if ( $paged > 1 ) {
        printf(
            '<a href="#" class="prev-next" data-page="%d" aria-label="%s">%s</a>',
            $paged - 1,
            esc_attr($prev_label),
            esc_html($prev_label)
        );
